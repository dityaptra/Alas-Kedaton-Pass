...
<section class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    ...
    {{-- Nomor Order --}}
    <div class="bg-green-700 text-white rounded-2xl p-6 text-center mb-6">
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-2xl p-6 mb-6">
        <h2 class="font-bold text-amber-800 mb-4">Cara Pembayaran</h2>
        <ol class="text-sm text-amber-700 space-y-3">
            <li class="flex gap-3">
                <span class="flex-shrink-0 w-6 h-6 bg-amber-500 text-white rounded-full
                             flex items-center justify-center text-xs font-bold">1</span>
                <span>Transfer sejumlah
                    <strong>Rp {{ number_format($order->total, 0, ',', '.') }}</strong>
                    ke rekening BCA <strong>1234567890</strong> a/n Pengelola Alas Kedaton.
                </span>
            </li>
            <li class="flex gap-3">
                <span class="flex-shrink-0 w-6 h-6 bg-amber-500 text-white rounded-full
                             flex items-center justify-center text-xs font-bold">2</span>
                <span>Kirim bukti transfer ke WhatsApp pengelola dengan format:<br>
                    <strong>NOMOR ORDER - NAMA</strong><br>
                    Contoh: <code class="bg-amber-100 px-1 rounded">{{ $order->order_number }} - {{ $order->visitor_name }}</code>
                </span>
            </li>
            <li class="flex gap-3">
                <span class="flex-shrink-0 w-6 h-6 bg-amber-500 text-white rounded-full
                             flex items-center justify-center text-xs font-bold">3</span>
                <span>E-ticket akan dikirim ke email
                    <strong>{{ $order->visitor_email }}</strong>
    ...
    {{-- Tombol WA --}}
    <a href="https://wa.me/{{ $waNumber }}?text={{ urlencode('Halo, saya ingin konfirmasi pembayaran. Nomor order: ' . $order->order_number . ' - ' . $order->visitor_name) }}"
       target="_blank"
       class="flex items-center justify-center gap-3 w-full bg-green-500
              hover:bg-green-600 text-white font-bold py-4 rounded-xl transition mb-4">
